...
namespace sudokuCLI
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var kivalasztottFeladatvany = nElemuFeladvany[index];

            Console.WriteLine("5. feladat: A kiválasztott feladvány:");
            Console.WriteLine(kivalasztottFeladatvany.Kezdo);

            double db = 0;
            foreach (char szamjegy in kivalasztottFeladatvany.Kezdo)
            {
                if (szamjegy != '0')
            { 
                    db++;
        ...
        Console.WriteLine("6. feladat: A feladvány kitöltöttsége: {0:f0}%", 100 * db / kivalasztottFeladvany.Kezdo.Length);
                ...
                kivalasztottFeladatvany.Kirajzol();
